...
### 2-5. ⚠️ 업로드 대상이 "공유 받은 폴더"면 이 설계는 성립하지 않는다

공식 문서가 명시한다 — **공유 받은 폴더와 암호 폴더는 Open API로 지원되지 않는다.**

이 앱의 README 첫 줄은 "양양 물고기반 **공유 폴더** 업로드용"이다. 여기서 말하는 공유 폴더가
무엇이냐에 따라 갈린다.

| 상황 | Open API 업로드 |
|---|---|
| 토큰 주인이 **소유한** 폴더 (본인이 만들어 남에게 공유해 준 폴더) | 가능할 것으로 보인다 |
| 토큰 주인이 **남에게서 공유받은** 폴더 | **불가** |

즉 **폴더 주인의 계정에서 발급한 토큰**을 쓰면 동작하고, 반원 각자의 계정 토큰으로
반장의 공유 폴더에 올리는 방식은 동작하지 않는다.

후자라면 선택지는 이렇게 된다.

1. 각자 자기 MYBOX에 올리고, 폴더를 반장에게 공유한다 (업로드 주체 = 폴더 소유자)
2. 반 공용 MYBOX 계정을 하나 두고 그 계정의 토큰을 공유한다
   (토큰이 드라이브 전체 권한이라 신중해야 한다)
3. MyBox 업로드는 폴더 소유자만 쓰고, 나머지는 기존처럼 리네임까지만 한다

**구현 전에 실제 폴더 구조를 확인해야 한다.** 소유 폴더로 확인되면 이 설계 그대로 간다.
